    ...
    <div class="crop-slider">
      <div ref="timeLineContainer" class="crop-time-line-container">
        ...
        >
          ...
          <div :class="getTimeHintClass(index)" :style="computedEndTimeIndicatorStyle(item)">
            <div class="cursor-line"></div>
        ...
        <div
          :class="['cursor-time-hint', {'moving-cursor': showNormalCursorTimeHint}]"
    ...
    <div class="crop-panel">
      <div class="crop-time-area">
        <div v-if="listLength > 1" class="crop-time-header">
          ...
          <button class="small-btn" @click="toggleShowList()">{{showList ? '收起' : '展开'}}</button>
        </div>
        <div
          :class="['crop-time-body', {'crop-time-body-hide': !showList}]"
        ...
        >
          <template v-for="(item, index) in renderList">
            ...
            >
              ...
              <button
                class="small-btn"
                @click="togglePlayClip(getOriginalIndex(index))"
              >{{playingIndex === getOriginalIndex(index) ? '暂停' : '播放'}}</button>
              <button class="small-btn" @click="removeCropItem(getOriginalIndex(index))">删除</button>
            </div>
          </template>
        </div>
      </div>
      ...
      >
        ...
        <button
          class="small-btn"
          @click="togglePlayClip(0)"
        >{{playing && playingIndex === 0 ? '暂停' : '播放'}}</button>
        <button class="small-btn" @click="addCropItem()">添加</button>
      </div>
      <div
        :class="['confirm-btn', {'confirm-btn-disabled': isSendingCrop}]"
        @click="confirmCrop()"
      >{{isSendingCrop ? language.SENDING_DATA :language.CONFIRM_CUT_VIDEO}}</div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    timeIndicatorCheck(currentCursorOffsetX, mouseEvent) {
      ...
      this.cropItemList.forEach((item, index) => {
        ...
        if (isCursorClose(item.endTimeIndicatorOffsetX, currentCursorOffsetX)) {
          ...
          isCursorClose(item.startTimeIndicatorOffsetX, currentCursorOffsetX)
        ) {
          this.startTimeIndicatorHoverIndex = index;
          // 鼠标放下，开始裁剪
          if (mouseEvent === "mousedown") {
    ...
    timeIndicatorMove(currentCursorOffsetX) {
      ...
      if (this.isCropping) {
        ...
        const startTimeIndicatorDraggingIndex = this
          .startTimeIndicatorDraggingIndex;
        const endTimeIndicatorDraggingIndex = this
          .endTimeIndicatorDraggingIndex;
        const currentCursorTime = this.currentCursorTime;
            ...
            if (
              ...
              if (this.isCropping) {
                ...
                this.timeIndicatorCheck(
                  currentCursorOffsetX < 0 ? 0 : containerWidth,
                  "mouseup"
                );
              }
              return;
